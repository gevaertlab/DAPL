...
def train(mo_initial,fracmissingselect,dataset_train,dataset_test,autoencoder_fun, restore=False,sav=True,checkpoint_file='default.ckpt'):
    ...
    with tf.Session() as session:
            ...
            for step in range(num_iters):
                ...
                sel=np.random.binomial(1,fracmissingselect, size=mo_initial.shape[0]*mo_initial.shape[1])
                sel=sel.reshape(mo_initial.shape[0],mo_initial.shape[1])
                missing_ones_fraction=mo_initial*sel
...
if __name__ == '__main__':
        ...
        gc=pd.read_csv("Average_GC.csv")
        CG_thres=gc.AV_GC_PCT.quantile(GC_quantile)
        gc_high=gc.loc[gc.AV_GC_PCT>CG_thres]
        gc_high_gene=gc_high.iloc[:,1]
        gene=df.columns.tolist()
        gene_df=pd.DataFrame(data=gene,columns=["Gene"])
        gc_high_df=gene_df[gene_df.Gene.apply(lambda sent: any(word in sent for word in gc_high_gene))]
        ind=gc_high_df.index.tolist()
        missing_ones_initial=np.zeros(shape=(batch_size,feature_size))
        missing_ones_initial[:,ind]=1
        print("GC high genes:",len(ind))
 

        np.set_printoptions(threshold=np.inf)
        tf.reset_default_graph()
        loss_val_list_train, loss_val_list_test=train(missing_ones_initial,fracmissingselect,dataset_train,dataset_test,autoencoder_fun=autoencoder4_d, sav=True,restore=False, checkpoint_file='imputationmodel_highGC.ckpt')  
        np.savetxt("trainloss_highGC.csv", loss_val_list_train, delimiter="\t")
        np.savetxt("validationloss_highGC.csv", loss_val_list_test, delimiter="\t")  
